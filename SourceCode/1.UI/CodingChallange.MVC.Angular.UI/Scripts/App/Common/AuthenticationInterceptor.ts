module Common.Interceptors
{
    export class AuthenticationInterceptor implements Common.Interfaces.IInterceptor
    {
        ...
        public request = ( config: any ): ng.IRequestConfig =>
        {
            ...
            var authData = this.localStorageService.get('authorizationData') as AdminSection.ViewModels.IAuthorizationVM;
            if ( authData )
            {
        ...
        public requestError = ( requestFailure: any ): ng.IPromise<any> =>
        {
            ...
            {
                var authService = this.injectorService.get('authService') as AdminSection.Interfaces.IAuthService;
                var authData = this.localStorageService.get('authorizationData') as AdminSection.ViewModels.IAuthorizationVM;
